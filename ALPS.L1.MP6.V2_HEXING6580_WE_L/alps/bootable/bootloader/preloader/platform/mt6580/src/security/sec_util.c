...
U32 sec_util_read_image (U8* img_name, U8 *buf, U64 offset, U32 size)
{
    ...
    {
        SMSG("[%s] part load '0x%x'\n", MOD, mt_part_get_start_addr(part));
        bDumpPartInfo = FALSE;
    }
    src = mt_part_get_start_addr(part) + offset;
    
    if (-1 == blkdev_read(bootdev, src, size, buf, mt_part_get_part_id(part)))
    {
...
static U32 sec_util_write_image (U8* img_name, U8 *buf, U64 offset, U32 size)
{
    ...
    if (-1 == blkdev_write(bootdev, dest, size, buf, mt_part_get_part_id(part)))
    {
