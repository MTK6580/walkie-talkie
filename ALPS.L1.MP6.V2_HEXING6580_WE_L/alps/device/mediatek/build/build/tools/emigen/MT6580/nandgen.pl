...
my $PLATFORM = $ENV{PLATFORM};# MTxxxx
...
{
    my $CUSTOM_MEMORYDEVICE_H_NAME  = "bootable/bootloader/preloader/custom/${PROJECT}/inc/custom_MemoryDevice.h";
...
{	my @all_column=[];#=qw(Vendor Part_Number Nand_ID AddrCycle IOWidth TotalSize_MB BlockSize_KB PageSize_B SpareSize_B Timing DQS_Delay S_Timing S_Timing1 Freq  CacheRead RandomRead Set_Feature	Get_Feature	Int_Address	Sync	Async	Rrtry_Addr	Rrtry_Number	Rrtry_Default	Rrtry_Start	PP_Function);
...
{
    my $NAND_LIST_DEFINE_H_NAME = "bootable/bootloader/preloader/custom/${PROJECT}/inc/nand_device_list.h";
    my $K_NAND_LIST_DEFINE_H_NAME = "kernel-3.10/drivers/misc/mediatek/mach/mt6580/${PROJECT}/nand/nand_device_list.h";
    my $LK_NAND_LIST_DEFINE_H_NAME = "bootable/bootloader/lk/target/${PROJECT}/inc/nand_device_list.h";
	my %InFileChip;
	my $Longest_ID=0;
	my $Chip_count=0;
    ...
    if(-e $K_NAND_LIST_DEFINE_H_NAME)
    {
        `chmod 777 $K_NAND_LIST_DEFINE_H_NAME`;
    }
    if(-e $LK_NAND_LIST_DEFINE_H_NAME)
    {
        `chmod 777 $LK_NAND_LIST_DEFINE_H_NAME`;
    }
...
#           if(search_PartNumber( $MemoryDeviceList[$iter]->{Part_Number} ) )
    ...
    foreach $ID (sort by_length (keys(%InFileChip))){
		...
		{
			print FD "\t{$arry_str, $InFileChip{$ID}->{IDLength},$MemoryDeviceList[$it]->{AddrCycle},$InFileChip{$ID}->{IO},$MemoryDeviceList[$it]->{TotalSize_MB},$MemoryDeviceList[$it]->{BlockSize_KB},$MemoryDeviceList[$it]->{PageSize_B},$MemoryDeviceList[$it]->{SpareSize_B},$MemoryDeviceList[$it]->{Timing}, $MemoryDeviceList[$it]->{DQS_Delay},$MemoryDeviceList[$it]->{S_Timing},$MemoryDeviceList[$it]->{S_Timing1},$MemoryDeviceList[$it]->{Freq},$InFileChip{$ID}->{FEATURE},$InFileChip{$ID}->{SECTOR}, ";
    ...
    print FD "#endif\n";
    close FD;
    `cp $NAND_LIST_DEFINE_H_NAME $LK_NAND_LIST_DEFINE_H_NAME`;
    `cp $NAND_LIST_DEFINE_H_NAME $K_NAND_LIST_DEFINE_H_NAME`;
#    `cp $NAND_LIST_DEFINE_H_NAME $K_M_NAND_LIST_DEFINE_H_NAME`;
