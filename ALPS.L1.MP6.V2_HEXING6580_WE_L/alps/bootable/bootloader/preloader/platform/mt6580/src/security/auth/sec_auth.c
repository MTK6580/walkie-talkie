...
int sec_auth (U8* data_buf, U32 data_len, U8* sig_buf, U32 sig_len)
{
    ...
    if( *(U32*)data_buf != (0x53535353) )
    {
        ...
        /* signature buffer is 256 bytes, which is 64 bytes sign image header + 
        128 bytes signature(RSA2048) + 20 bytes hash(SHA1) + 44 bytes 0 padding. */
 ...
 *  IMAGE AUTH INIT
 **************************************************************************/
/* When RSA1024 is no longer used, this function call should be removed */
