...
sub Parse_MD_Struct
{
	...
	seek(MODEM, $md_file_size - 4, $whence) or die;
	...
	{
		$length1 = $size;
		seek(MODEM, $md_file_size - $length3 - $length4 - $length5 - $length1, $whence) or die "Can NOT seek to the position of modem image rear in \"$parse_md_file\"!\n";
...
sub find_modem_bin
{
	...
	elsif (scalar @wildcard_list == 0)
	{
		...
		if (-e $path_of_bin . "cp.rom")
		{
			...
			$ref_hash_feature->{"TYPE_OF_BIN"} |= 0b00010000;
		}
	}
	else
	{
...
sub get_modem_file_mapping
{
	...
	if ($ref_hash_filelist)
	{
		...
		elsif ($ref_hash_feature->{"TYPE_OF_BIN"} == 0b00010000)
		{
			# C2K
			my $path_of_fsm;
			my $path_of_rom;
			...
			push(@checklist_of_general, $path_of_fsm . "fsm_rf_df.img");
			push(@checklist_of_general, $path_of_fsm . "fsm_rw_df.img");
			push(@checklist_of_general, $path_of_fsm . "fsm_cust_df.img");
		}
		elsif (($ref_hash_feature->{"image_type"} >= 5) and ($ref_hash_feature->{"image_type"} <= 7))
		{
			...
			push(@checklist_of_append, $path_of_database . "MDDB_*");
...
$(foreach x,1 2 5,\
  $(if $(filter yes,$(strip $(MTK_ENABLE_MD$(x)))),\
    $(foreach yy,$(MTK_MODEM_MAP_X_$(x)_TO_YY),\
      ...
      $(eval MTK_MODEM_MDDB_FILES += $(notdir $(wildcard $(MTK_MODEM_LOCAL_PATH)/MDDB_*_$(x)_$(yy)_*)))\
